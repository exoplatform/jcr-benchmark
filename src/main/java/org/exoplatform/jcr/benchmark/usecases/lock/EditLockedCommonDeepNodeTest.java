   ...
   extends JCRTestBase
{
   ...
   {
      Node rootNode =
               (Node) context.getSession().getItem("/" + rootNodeName + "/" + child1 + "/" + child2 + "/" + child3);
      try
      {
   ...
   private synchronized void createRootNode(JCRTestContext context)
   {
      try
      {
         if (!rootNodeCreated)
         {
            ...
            rootNode.addNode(child1, "nt:unstructured").addNode(child2, "nt:unstructured").addNode(child3,
                     "nt:unstructured");
            tmpSession.save();
            tmpSession.logout();
